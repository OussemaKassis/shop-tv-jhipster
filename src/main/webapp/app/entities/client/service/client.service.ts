...
type RestOf<T extends IClient | NewClient> = Omit<T, 'dateOfBirth'> & {
...
export class ClientService {
  ...
  protected convertDateFromClient<T extends IClient | NewClient | PartialUpdateClient>(client: T): RestOf<T> {
    return {
      ...client,
      dateOfBirth: client.dateOfBirth?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restClient: RestClient): IClient {
    return {
      ...restClient,
      dateOfBirth: restClient.dateOfBirth ? dayjs(restClient.dateOfBirth) : undefined,
    };
  }
