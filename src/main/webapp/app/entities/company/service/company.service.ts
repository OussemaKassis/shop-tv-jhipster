...
type RestOf<T extends ICompany | NewCompany> = Omit<T, 'companyCreationDate'> & {
...
export class CompanyService {
  ...
  protected convertDateFromClient<T extends ICompany | NewCompany | PartialUpdateCompany>(company: T): RestOf<T> {
    return {
      ...company,
      companyCreationDate: company.companyCreationDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCompany: RestCompany): ICompany {
    return {
      ...restCompany,
      companyCreationDate: restCompany.companyCreationDate ? dayjs(restCompany.companyCreationDate) : undefined,
    };
  }
