...
type RestOf<T extends IAddress | NewAddress> = Omit<T, 'zipCode'> & {
  zipCode?: string | null;
};
...
export class AddressService {
  ...
  protected convertDateFromClient<T extends IAddress | NewAddress | PartialUpdateAddress>(address: T): RestOf<T> {
    return {
      ...address,
      zipCode: address.zipCode?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAddress: RestAddress): IAddress {
    return {
      ...restAddress,
      zipCode: restAddress.zipCode ? dayjs(restAddress.zipCode) : undefined,
    };
  }
